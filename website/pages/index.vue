<template>
  <div class="pb-16">
    <div class="bg-forest-night pb-24">
      <div class="pt-24 pb-10 px-4 sm:px-6 lg:px-8">
        <div class="text-center">
          ...
          <p class="max-w-xl mt-5 mx-auto text-xl leading-7 text-rainy-grey">Discover our list of modules to supercharge your <a href="https://nuxtjs.org" class="border-b border-stone-green hover:text-green-500 hover:border-green-600">Nuxt project</a>.<br/>Created by the Nuxt team and community.</p>
    ...
    <div class="py-12 container mx-auto">
      <div class="sm:max-w-lg sm:mx-auto flex border border-rainy-grey rounded-md overflow-hidden shadow-sm">
        <input v-model="q" v-focus aria-label="Search" class="appearance-none block w-full px-3 py-3 text-base leading-6 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 transition duration-150 ease-in-out sm:flex-1" placeholder="Search a module (name, category, username, etc.)">
        <button class="mt-3 w-full px-6 py-3 bg-rainy-grey hover:bg-grey-light text-gray-700 text-base leading-6 font-medium shadow-sm focus:outline-none transition duration-150 ease-in-out sm:mt-0 sm:flex-shrink-0 sm:inline-flex sm:items-center sm:w-auto">
        ...
        <div v-for="module of filteredModules" :key="module.name" class="relative flex flex-col bg-white shadow rounded-md overflow-hidden hover:shadow-lg">
          ...
          <div class="border-t border-gray-200 bg-gray-100 grid grid-cols-3">
            ...
            <a :href="module.github" target=" _blank" rel="noopener" class="group flex items-center space-x-1 py-3 px-4 border-r border-gray-200">
              ...
              <div class="text-sm leading-5 text-gray-600 group-hover:text-gray-900 font-medium">{{ numberFormat(module.stars) }} star{{ module.stars !== 1 ? 's' : '' }}</div>
            </a>
